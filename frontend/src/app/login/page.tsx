...
export default function LoginPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (phone.trim().length >= 10) {
      router.push(`/login/verificar/?phone=${encodeURIComponent(phone)}`);
    }
  }
    ...
    <div className="min-h-dvh bg-bg-primary flex flex-col items-center px-5 sm:px-8">
      ...
      >
        ...
        <input
          ...
          className="w-full h-11 sm:h-12 rounded-pill bg-bg-card-highlight text-white px-5 text-sm outline-none focus:ring-2 focus:ring-accent/40 placeholder:text-text-muted"
        />
