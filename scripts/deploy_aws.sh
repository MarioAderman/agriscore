...
# ECR
ECR_CORE="${ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${PROJECT}-core"
ECR_API_REST="${ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${PROJECT}-api-rest"
ECR_EVOLUTION="${ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${PROJECT}-evolution"

# ECS
CLUSTER="${PROJECT}-cluster"
CORE_SERVICE="${PROJECT}-core"
API_REST_SERVICE="${PROJECT}-api-rest"
...
if [[ "${1:-}" == "teardown" ]]; then
  ...
  aws ecs update-service --cluster $CLUSTER --service $CORE_SERVICE --desired-count 0 --region $REGION 2>/dev/null || true
  aws ecs update-service --cluster $CLUSTER --service $API_REST_SERVICE --desired-count 0 --region $REGION 2>/dev/null || true
  ...
  aws ecs delete-service --cluster $CLUSTER --service $CORE_SERVICE --force --region $REGION 2>/dev/null || true
  aws ecs delete-service --cluster $CLUSTER --service $API_REST_SERVICE --force --region $REGION 2>/dev/null || true
  ...
  for TG_NAME in ${PROJECT}-core-tg ${PROJECT}-api-rest-tg ${PROJECT}-evolution-tg; do
...
aws ecr describe-repositories --repository-names ${PROJECT}-core --region $REGION 2>/dev/null \
  || aws ecr create-repository --repository-name ${PROJECT}-core --region $REGION
log "ECR: ${PROJECT}-core"

aws ecr describe-repositories --repository-names ${PROJECT}-api-rest --region $REGION 2>/dev/null \
  || aws ecr create-repository --repository-name ${PROJECT}-api-rest --region $REGION
log "ECR: ${PROJECT}-api-rest"
...
docker build -f infra/docker/Dockerfile.core -t ${ECR_CORE}:latest .
docker push ${ECR_CORE}:latest
docker build -f infra/docker/Dockerfile.api-rest -t ${ECR_API_REST}:latest .
docker push ${ECR_API_REST}:latest
log "Pushed ${PROJECT}-core and ${PROJECT}-api-rest images"
...
CORE_TG=$(aws elbv2 describe-target-groups --names ${PROJECT}-core-tg --query 'TargetGroups[0].TargetGroupArn' --output text --region $REGION 2>/dev/null || echo "None")
if [[ "$CORE_TG" == "None" || -z "$CORE_TG" ]]; then
  CORE_TG=$(aws elbv2 create-target-group --name ${PROJECT}-core-tg --protocol HTTP --port 8000 \
    --vpc-id $VPC_ID --target-type ip --health-check-path /docs \
    --query 'TargetGroups[0].TargetGroupArn' --output text --region $REGION)
fi
log "Target group: Core → $CORE_TG"

API_REST_TG=$(aws elbv2 describe-target-groups --names ${PROJECT}-api-rest-tg --query 'TargetGroups[0].TargetGroupArn' --output text --region $REGION 2>/dev/null || echo "None")
if [[ "$API_REST_TG" == "None" || -z "$API_REST_TG" ]]; then
  API_REST_TG=$(aws elbv2 create-target-group --name ${PROJECT}-api-rest-tg --protocol HTTP --port 8000 \
    --vpc-id $VPC_ID --target-type ip --health-check-path /health \
    --query 'TargetGroups[0].TargetGroupArn' --output text --region $REGION)
fi
log "Target group: API REST → $API_REST_TG"
...
if [[ "$LISTENER_ARN" == "None" || -z "$LISTENER_ARN" ]]; then
  LISTENER_ARN=$(aws elbv2 create-listener --load-balancer-arn $ALB_ARN --protocol HTTP --port 80 \
    --default-actions Type=forward,TargetGroupArn=$CORE_TG \
    --query 'Listeners[0].ListenerArn' --output text --region $REGION)
fi
log "Listener: port 80 → Core (default)"

# Path-based routing: /api/customer/* and /api/farmer/* → API REST service
aws elbv2 create-rule --listener-arn $LISTENER_ARN --priority 10 \
  --conditions '[{"Field":"path-pattern","Values":["/api/customer/*","/api/farmer/*"]}]' \
  --actions Type=forward,TargetGroupArn=$API_REST_TG \
  --region $REGION 2>/dev/null || true
log "ALB rule: /api/customer/*, /api/farmer/* → API REST"
...
# Create task role if needed (permissions for the running containers)
TASK_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/${PROJECT}-task-role"
if ! aws iam get-role --role-name ${PROJECT}-task-role 2>/dev/null; then
  aws iam create-role --role-name ${PROJECT}-task-role \
    --assume-role-policy-document '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"ecs-tasks.amazonaws.com"},"Action":"sts:AssumeRole"}]}'
  aws iam put-role-policy --role-name ${PROJECT}-task-role --policy-name task-permissions \
    --policy-document "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":\"states:StartExecution\",\"Resource\":\"arn:aws:states:${REGION}:${ACCOUNT_ID}:stateMachine:${PROJECT}-pipeline\"},{\"Effect\":\"Allow\",\"Action\":[\"s3:GetObject\",\"s3:PutObject\"],\"Resource\":\"arn:aws:s3:::${S3_BUCKET}/*\"},{\"Effect\":\"Allow\",\"Action\":[\"bedrock:InvokeModel\",\"bedrock:InvokeModelWithResponseStream\"],\"Resource\":\"*\"},{\"Effect\":\"Allow\",\"Action\":\"sagemaker:InvokeEndpoint\",\"Resource\":\"arn:aws:sagemaker:${REGION}:${ACCOUNT_ID}:endpoint/${PROJECT}-endpoint\"}]}"
  sleep 5
  log "Created ECS task role"
else
  log "ECS task role exists"
fi

# Task definition: Core (Agent + Pipeline)
cat > /tmp/core-task-def.json <<TASKEOF
{
  "family": "${PROJECT}-core",
  "networkMode": "awsvpc",
  ...
  "containerDefinitions": [
    {
      "name": "core",
      "image": "${ECR_CORE}:latest",
      "portMappings": [{"containerPort": 8000, "protocol": "tcp"}],
      "environment": [
        {"name": "ENVIRONMENT", "value": "production"},
        {"name": "DATABASE_URL", "value": "${DATABASE_URL:-REPLACE_WITH_RDS_URL}"},
        {"name": "EVOLUTIONAPI_URL", "value": "${EVOLUTIONAPI_URL:-http://localhost:8080}"},
        ...
        {"name": "SENTINEL_HUB_ID", "value": "${SENTINEL_HUB_ID:-}"},
        {"name": "SENTINEL_HUB_SECRET", "value": "${SENTINEL_HUB_SECRET:-}"},
        {"name": "INEGI_TOKEN", "value": "${INEGI_TOKEN:-}"},
        {"name": "SAGEMAKER_ENDPOINT", "value": "${PROJECT}-endpoint"},
        {"name": "EVOLUTIONAPI_AUTHENTICATION_API_KEY", "value": "${EVOLUTIONAPI_AUTHENTICATION_API_KEY:-}"},
        {"name": "EVOLUTION_INSTANCE_NAME", "value": "${EVOLUTION_INSTANCE_NAME:-Fintegra solutions}"}
      ],
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/${PROJECT}-core",
...
aws ecs register-task-definition --cli-input-json file:///tmp/core-task-def.json --region $REGION > /dev/null
log "Task definition: ${PROJECT}-core"

# Task definition: API REST (Customer + Farmer endpoints)
cat > /tmp/api-rest-task-def.json <<TASKEOF
{
  "family": "${PROJECT}-api-rest",
  "networkMode": "awsvpc",
  "requiresCompatibilities": ["FARGATE"],
  "cpu": "256",
  "memory": "512",
  ...
  "containerDefinitions": [
    {
      "name": "api-rest",
      "image": "${ECR_API_REST}:latest",
      "portMappings": [{"containerPort": 8000, "protocol": "tcp"}],
      "environment": [
        {"name": "ENVIRONMENT", "value": "production"},
        {"name": "DATABASE_URL", "value": "${DATABASE_URL:-REPLACE_WITH_RDS_URL}"},
        {"name": "CUSTOMER_API_KEY", "value": "${CUSTOMER_API_KEY:-demo-customer-key}"},
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/${PROJECT}-api-rest",
          "awslogs-region": "${REGION}",
          "awslogs-stream-prefix": "ecs",
          "awslogs-create-group": "true"
        }
...
aws ecs register-task-definition --cli-input-json file:///tmp/api-rest-task-def.json --region $REGION > /dev/null
log "Task definition: ${PROJECT}-api-rest"
  ...
  "containerDefinitions": [
    {
      ...
      "environment": [
        {"name": "AUTHENTICATION_API_KEY", "value": "${EVOLUTIONAPI_AUTHENTICATION_API_KEY:-}"},
        {"name": "DATABASE_ENABLED", "value": "true"},
        {"name": "DATABASE_PROVIDER", "value": "postgresql"},
        {"name": "DATABASE_CONNECTION_URI", "value": "${DATABASE_URL:-}"}
      ],
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
...
aws ecs describe-services --cluster $CLUSTER --services $CORE_SERVICE --query 'services[?status==`ACTIVE`].serviceName' --output text --region $REGION 2>/dev/null | grep -q $CORE_SERVICE \
  || aws ecs create-service --cluster $CLUSTER --service-name $CORE_SERVICE \
    --task-definition ${PROJECT}-core --desired-count 1 --launch-type FARGATE \
    --network-configuration "awsvpcConfiguration={subnets=[$FIRST_SUBNET],securityGroups=[$SG_API],assignPublicIp=ENABLED}" \
    --load-balancers "targetGroupArn=$CORE_TG,containerName=core,containerPort=8000" \
    --region $REGION > /dev/null
log "ECS service: $CORE_SERVICE"

aws ecs describe-services --cluster $CLUSTER --services $API_REST_SERVICE --query 'services[?status==`ACTIVE`].serviceName' --output text --region $REGION 2>/dev/null | grep -q $API_REST_SERVICE \
  || aws ecs create-service --cluster $CLUSTER --service-name $API_REST_SERVICE \
    --task-definition ${PROJECT}-api-rest --desired-count 1 --launch-type FARGATE \
    --network-configuration "awsvpcConfiguration={subnets=[$FIRST_SUBNET],securityGroups=[$SG_API],assignPublicIp=ENABLED}" \
    --load-balancers "targetGroupArn=$API_REST_TG,containerName=api-rest,containerPort=8000" \
    --region $REGION > /dev/null
log "ECS service: $API_REST_SERVICE"
...
# Always update inline policy (idempotent) — S3 read + SageMaker invoke
aws iam put-role-policy --role-name ${PROJECT}-lambda-role --policy-name lambda-permissions \
  --policy-document "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Effect\":\"Allow\",\"Action\":[\"s3:GetObject\"],\"Resource\":\"arn:aws:s3:::${S3_BUCKET}/*\"},{\"Effect\":\"Allow\",\"Action\":\"sagemaker:InvokeEndpoint\",\"Resource\":\"arn:aws:sagemaker:${REGION}:${ACCOUNT_ID}:endpoint/${PROJECT}-endpoint\"}]}"
LAMBDA_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/${PROJECT}-lambda-role"

# Shared env vars for all Lambdas (Settings uses extra="ignore", .env loaded at top)
LAMBDA_ENV_VARS="DATABASE_URL=${DATABASE_URL:-},ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY:-},SENTINEL_HUB_ID=${SENTINEL_HUB_ID:-},SENTINEL_HUB_SECRET=${SENTINEL_HUB_SECRET:-},INEGI_TOKEN=${INEGI_TOKEN:-},GROQ_API_KEY=${GROQ_API_KEY:-},OPENAI_API_KEY=${OPENAI_API_KEY:-},S3_BUCKET=${S3_BUCKET},EVOLUTIONAPI_URL=${EVOLUTIONAPI_URL:-http://localhost:8080},EVOLUTIONAPI_AUTHENTICATION_API_KEY=${EVOLUTIONAPI_AUTHENTICATION_API_KEY:-},EVOLUTION_INSTANCE_NAME=${EVOLUTION_INSTANCE_NAME:-Fintegra solutions},SAGEMAKER_ENDPOINT=${PROJECT}-endpoint"
...
LAMBDA_CONFIGS=(
  ...
  "calculate-score:calculate_score:256:30:common"
